...
class IWnoteboard_Api_Admin extends Zikula_Api {
    /**
     * Create a new topic
    ...
    public function createShared($args) {
        ...
        if (!DBUtil::insertObject($item, 'IWnoteboard_public', 'pid')) {
            return LogUtil::registerError($this->__('Error! Creation attempt failed.'));
        }
     ...
     * @return:	true if success and false if fails
     */
    public function delShared($args) {
        ...
        // Get the item
        $item = ModUtil::apiFunc('IWnoteboard', 'user', 'getShared', array('pid' => $pid));
        if (!$item) {
            return LogUtil::registerError($this->__('No such item found.'));
        }

        if (!DBUtil::deleteObjectByID('IWnoteboard_public', $pid, 'pid')) {
            return LogUtil::registerError($this->__('Error! Sorry! Deletion attempt failed.'));
        }

        // The item has been deleted, so we clear all cached pages of this item.
        $view = Zikula_View::getInstance('IWnoteboard');
        $view->clear_cache(null, $pid);
    ...
    public function editShared($args) {
        ...
        if (!DBUTil::updateObject($items, 'IWnoteboard_public', $where)) {
            return LogUtil::registerError($this->__('Error! Update attempt failed.'));
        }
